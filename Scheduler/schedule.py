...
def assign_weekday(current_date, personas, assignments, errors):
    ...
    # Only pick those who match working_day, next_available <= current_date, and can_work_day
    candidates = [
        p
        for p in personas
        if p["working_day"] == day_name
        and p["next_available"] <= current_date
        and can_work_day(p, current_date)
    ]
    if not candidates:
        assignments.append(
            {
    ...
    # Build pairs that are not incompatible
    pairs = []
...
def assign_weekend(current_date, personas, assignments, errors, weekend_counts):
    ...
    candidates = [
        p
        for p in personas
        if p["next_available"] <= current_date and can_work_day(p, current_date)
    ]
    candidates.sort(key=lambda x: x["weekend_shifts_count"])

    assigned_pair = None
    min_count = candidates[0]["weekend_shifts_count"] if candidates else 0
    group_min = [p for p in candidates if p["weekend_shifts_count"] == min_count]

    if len(group_min) < 2:
        group_min = candidates
